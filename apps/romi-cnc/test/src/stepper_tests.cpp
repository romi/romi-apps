...
class stepper_tests : public ::testing::Test
{
protected:
	...
	stepper_tests() : range(xmin, xmax) {}

	~stepper_tests() override = default;
...
TEST_F(stepper_tests, test_homing_romiserial)
{
...
TEST_F(stepper_tests, test_homing_stepper)
{
        RSerial serial("/dev/ttyACM0", 115200, 1);        
        RomiSerialClient romi_serial(&serial, &serial);
...
TEST_F(stepper_tests, test_move)
{
        RSerial serial("/dev/ttyACM0", 115200, 1);        
        RomiSerialClient romi_serial(&serial, &serial);
        StepperController stepper(romi_serial);
...
TEST_F(stepper_tests, test_get_position)
{
        RSerial serial("/dev/ttyACM0", 115200, 1);        
        RomiSerialClient romi_serial(&serial, &serial);
        StepperController stepper(romi_serial);
